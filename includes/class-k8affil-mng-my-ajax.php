...
class K8affil_Mng_My_Ajax
{
	...
	function __construct( $args )
	{
		...
		#Sync Routers CF data
		add_action('wp_ajax_nopriv_k8affil_act_rou', array( $this, 'k8affil_act_rou' ));
		add_action('wp_ajax_k8affil_act_rou', array( $this, 'k8affil_act_rou' ));
	}
	...
	public function k8affil_act_amb(){
	 ...
	 #Sync Routers CF data
	public function k8affil_act_rou(){
		$arrr = array();
		extract( $_POST );
		#Sent not from website
		if( !isset( $action ) || $action != 'k8affil_act_rou' ){
			$arrr['error'] = 'Submit via website, please';
			$this->finn($arrr);
		}

		$res =  wp_remote_get( 'https://vpn-anbieter-vergleich-test.de/wp-json/m5-route/m5-routers/' );
		$decc = json_decode( $res['body'], true );
		...
		if ( $the_query->have_posts() ) :
			while ( $the_query->have_posts() ) : $the_query->the_post();
				$pid = get_the_ID();
				$m5_rou_id = (int)get_field('m5_rou_id', $pid);

				#There is no such a router with that ID
				if(!isset($decc[$m5_rou_id]))
					continue;

				#Update ACF Fields
				foreach ($cust_fields as $k):

					#For Single Router image
					if( $k == 'm5_rou_id_prod_img' ){
						#On original website (vavt.de) single router image is not set, so quit
						if( !is_array( $decc[$m5_rou_id]['cust_fields'][$k] ) || count( $decc[$m5_rou_id]['cust_fields'][$k] ) == 0 )
							continue;
						#Image previously uploaded - so update existing
						$imgg_id = K8affil_Mng_My_Help::doesUpl2( $decc[$m5_rou_id]['cust_fields'][$k]['filename'] );
						#if image didn't upload before - so upload now
						if( $imgg_id == 0 ){
							$imgg_id = K8affil_Mng_My_Help::uplImg( $decc[$m5_rou_id]['cust_fields'][$k]['url'], $pid );
						}
						update_field( $k, $imgg_id, $pid );
						continue;
					}

					#For Gallery of Images
					if( $k == 'm5_rou_id_var_img' ){
						#On original website (vavt.de) image gallery is not set, so quit
						if( !is_array( $decc[$m5_rou_id]['cust_fields'][$k] ) || count( $decc[$m5_rou_id]['cust_fields'][$k] ) == 0 )
							continue;

						$img_arr = [];
						foreach ($decc[$m5_rou_id]['cust_fields'][$k] as $img) {
							#if image was uploaded before
							$imgg_id = K8affil_Mng_My_Help::doesUpl2( $img['filename'] );
							#if image didn't upload before - so upload now
							if( $imgg_id == 0 ){
								$imgg_id = K8affil_Mng_My_Help::uplImg( $img['url'], $pid );
							}
							$img_arr[] = $imgg_id;
						}
						update_field( $k, $img_arr, $pid );
						continue;
					}

					#if is Checkbox or Select
					if( is_array( $decc[$m5_rou_id]['cust_fields'][$k] ) && count( $decc[$m5_rou_id]['cust_fields'][$k] ) > 0 && !isset( $decc[$m5_rou_id]['cust_fields'][$k]['value'] ) ){
						$vals = array();
						foreach ($decc[$m5_rou_id]['cust_fields'][$k] as $it) {
							$vals[] = $it['value'];
						}
						update_field( $k, $vals, $pid );
						continue;
					...
					#if Radio Button
					if( is_array( $decc[$m5_rou_id]['cust_fields'][$k] ) && count( $decc[$m5_rou_id]['cust_fields'][$k] ) > 0 && isset( $decc[$m5_rou_id]['cust_fields'][$k]['value'] ) ){
						update_field( $k, $decc[$m5_rou_id]['cust_fields'][$k]['value'], $pid );
						continue;
					}

					update_field( $k, $decc[$m5_rou_id]['cust_fields'][$k], $pid );
				endforeach;
					// END Update ACF Fields
			endwhile;
			wp_reset_postdata();
